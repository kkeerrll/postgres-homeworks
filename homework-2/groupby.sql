...
select country, count(*) as count_country from suppliers group by country order by count_country desc;
...
select ship_country, sum(freight) as sum_freight from orders where ship_region is not NULL group by ship_country having суммарный_вес > 2750 order by суммарный_вес desc;
